...
namespace MyriadMusicTagger.Utils
{
    ...
    public class AudioQualityProcessor : IDisposable
    {
        ...
        private const int MaxSamplesPerChunk = 44100 * 30; // 30 seconds at 44.1kHz
        private const int MaxTotalSamples = 44100 * 300; // 5 minutes max for analysis
        private const float QuietThreshold = 0.01f; // -40dB roughly
        private const float Mp3RolloffThreshold = 0.707f; // -3dB
        private const int HighFrequencyStartHz = 15000;
        private const int SpectralAnalysisStartHz = 10000;
        private const double MinDelayBetweenRequests = 0.2; // Minimum delay in seconds
        
        // Frequency Analysis Constants
        private const int Mp3Cutoff16kHz = 16000;
        private const int Mp3Cutoff20kHz = 20000;
        private const int Mp3Cutoff14kHz = 14000;
        
        // Dynamic Range Constants
        private const float MinDynamicRangeDb = 3.0f;
        private const float LowDynamicRangeDb = 4.0f;
        private const float AcceptableDynamicRangeDb = 6.0f;
        private const float ExcellentDynamicRangeDb = 15.0f;
        
        // Clipping Constants
        private const float SignificantClippingPercent = 5.0f;
        private const float ModerateClippingPercent = 3.0f;
        private const float NormalClippingPercent = 3.0f; // Below this is considered normal
        private const float SustainedClippingThresholdMs = 10.0f;
        private const float ExtremePeakThreshold = 1.0f; // +1 dBFS
        
        // Noise Floor Constants
        private const float PoorNoiseFloorDb = -30.0f;
        private const float AcceptableNoiseFloorDb = -35.0f;
        private const float NoiseVariationThreshold = 10.0f;
        
        // Channel Quality Constants
        private const float MonoThreshold = 0.01f; // 1% differences = mono
        private const float PseudoStereoCorrelation = 0.95f;
        private const float MonoCorrelationThreshold = 0.999f;
        private const float PhaseIssueThreshold = -0.5f;
        private const float ChannelImbalanceThreshold = 0.2f; // 20% imbalance
        private const float SeverePhaseIssueThreshold = -0.8f;
        private const float NarrowStereoThreshold = 0.05f;
        ...
        public async Task<AudioQualityResult> AnalyzeAudioQualityAsync(string filePath, int mediaId, string title, string artist)
        {
            ...
            try
            {
                ...
                // Set dynamic recommendation flag using configured threshold
                result.RecommendReRip = result.OverallQualityScore < _settings.ReRipThreshold;
        ...
        private async Task<AudioData?> LoadAudioDataAsync(string filePath)
        {
            return await Task.Run(() =>
            {
                try
                {
                    return LoadAudioDataCore(filePath);
                }
                catch (Exception ex)
                {
        ...
        /// <summary>
        /// Core audio loading implementation
        /// </summary>
        private AudioData? LoadAudioDataCore(string filePath)
        {
            ...
            try
            {
                ...
                return ProcessAudioInChunks(stream, info, lengthSamples, filePath);
            }
            finally
            {
        ...
        /// <summary>
        /// Processes audio data in chunks to manage memory usage
        /// </summary>
        private AudioData? ProcessAudioInChunks(int stream, ChannelInfo info, int lengthSamples, string filePath)
        {
            ...
            try
            {
                while (remainingSamples > 0)
                {
                    ...
                    ExtractChannelData(chunkBuffer, leftChannel, rightChannel, actualSamplesThisChunk, info.Channels);
                    
                    currentPosition += actualSamplesThisChunk;
                    remainingSamples -= actualSamplesThisChunk;
                    
                    // Log progress for very large files
                    LogProcessingProgress(totalSamples, remainingSamples);
        ...
        /// <summary>
        /// Extracts channel data from interleaved audio buffer
        /// </summary>
        private static void ExtractChannelData(float[] chunkBuffer, List<float> leftChannel, List<float> rightChannel, 
            int actualSamplesThisChunk, int channels)
        {
            for (int i = 0; i < actualSamplesThisChunk; i++)
            {
                leftChannel.Add(chunkBuffer[i * channels]);
                if (channels > 1)
                {
                    rightChannel.Add(chunkBuffer[i * channels + 1]);
                }
                else
                {
                    rightChannel.Add(chunkBuffer[i * channels]); // Mono
                }
            }
        }

        /// <summary>
        /// Logs processing progress for large files
        /// </summary>
        private static void LogProcessingProgress(int totalSamples, int remainingSamples)
        {
        ...
        private async Task<SpectralAnalysisResult> AnalyzeSpectralQualityAsync(AudioData audioData)
        {
            return await Task.Run(() =>
            {
                ...
                try
                {
                    ...
                    var monoData = CombineChannelsToMono(audioData);
                    var spectrums = ProcessSpectralWindows(monoData, sampleRate);
                    ...
                    var avgSpectrum = CalculateAverageSpectrum(spectrums);
        ...
        /// <summary>
        /// Combines stereo channels to mono for spectral analysis
        /// </summary>
        private static float[] CombineChannelsToMono(AudioData audioData)
        {
        ...
        /// <summary>
        /// Processes overlapping FFT windows for spectral analysis
        /// </summary>
        private static List<double[]> ProcessSpectralWindows(float[] monoData, int sampleRate)
        {
            var spectrums = new List<double[]>();
            var window = MathNet.Numerics.Window.Hann(DefaultFftSize);
            int hopSize = DefaultFftSize / 4;

            // Process overlapping windows
            for (int pos = 0; pos + DefaultFftSize < monoData.Length; pos += hopSize)
            {
                var fftData = new Complex[DefaultFftSize];
                ...
                var spectrum = new double[DefaultFftSize / 2];
        ...
        /// <summary>
        /// Calculates the average spectrum from all processed windows
        /// </summary>
        private static double[] CalculateAverageSpectrum(List<double[]> spectrums)
        {
            var avgSpectrum = new double[DefaultFftSize / 2];
        ...
        private void AnalyzeFrequencyContent(SpectralAnalysisResult result, double[] spectrum, int sampleRate)
        {
            ...
            var rolloffThreshold = peakEnergy * Mp3RolloffThreshold; // -3dB
            ...
            int hfStartBin = (int)(HighFrequencyStartHz / freqPerBin);
        ...
        private void DetectMp3Artifacts(SpectralAnalysisResult result, double[] spectrum, int sampleRate)
        {
            ...
            var cutoff16k = (int)(Mp3Cutoff16kHz / freqPerBin);
            var cutoff20k = (int)(Mp3Cutoff20kHz / freqPerBin);
            var cutoff14k = (int)(Mp3Cutoff14kHz / freqPerBin);

            if (cutoff16k < spectrum.Length && cutoff20k < spectrum.Length)
            {
                var energy16k = spectrum.Skip(cutoff16k).Take(100).Average();
                var energy14k = spectrum.Skip(cutoff14k).Take(100).Average();
                ...
                if (energy14k > 0 && energy16k / energy14k < 0.1)
                {
                    ...
                    result.Issues.Add($"Sharp frequency cutoff at ~{Mp3Cutoff16kHz}Hz suggests MP3 encoding");
                    result.SuspiciousFrequencies.Add(Mp3Cutoff16kHz);
                }
        ...
        private void CalculateSpectralMetrics(SpectralAnalysisResult result, double[] spectrum, int sampleRate)
        {
            ...
            if (totalEnergy > 0)
            {
                ...
                int hfStart = (int)(SpectralAnalysisStartHz / freqPerBin); // Above 10kHz
        ...
        private DynamicRangeResult AnalyzeDynamicRange(AudioData audioData)
        {
            ...
            {
                // Combine channels for analysis
                var monoData = CombineChannelsForDynamicRange(audioData);

                // Calculate peak and RMS levels
                CalculateLevels(result, monoData);
                ...
                // Score and assess dynamic range
                ScoreDynamicRange(result);
        ...
        /// <summary>
        /// Combines audio channels for dynamic range analysis
        /// </summary>
        private static float[] CombineChannelsForDynamicRange(AudioData audioData)
        {
            var monoData = new float[audioData.LeftChannel.Length];
            for (int i = 0; i < monoData.Length; i++)
            {
        ...
        /// <summary>
        /// Calculates peak and RMS levels for dynamic range analysis
        /// </summary>
        private static void CalculateLevels(DynamicRangeResult result, float[] monoData)
        {
        ...
        /// <summary>
        /// Scores dynamic range quality and adds appropriate issues
        /// </summary>
        private static void ScoreDynamicRange(DynamicRangeResult result)
        {
            // Score based on dynamic range - adjusted for modern mastering practices
            result.DynamicRangeScore = Math.Min(100, Math.Max(0, (float)((result.DynamicRange - MinDynamicRangeDb) * 8.33))); // 3dB = 0 points, 15dB = 100 points

            // Penalties for extreme over-compression (adjusted thresholds)
            if (result.DynamicRange < LowDynamicRangeDb)
            {
            ...
            else if (result.DynamicRange < AcceptableDynamicRangeDb)
            {
        ...
        private ClippingAnalysisResult AnalyzeClipping(AudioData audioData)
        {
            ...
            try
            {
                ...
                CombineClippingResults(result, leftClipping, rightClipping, audioData);
                ...
                // Calculate clipping penalties and issues
                AssessClippingQuality(result, audioData.SampleRate);
        ...
        /// <summary>
        /// Combines clipping detection results from both channels
        /// </summary>
        private static void CombineClippingResults(ClippingAnalysisResult result, 
            (int ClippedSamples, int ClippingEvents, int MaxConsecutive) leftClipping,
            (int ClippedSamples, int ClippingEvents, int MaxConsecutive) rightClipping,
            AudioData audioData)
        {
            ...
            var sustainedThreshold = audioData.SampleRate * SustainedClippingThresholdMs / 1000f; // 10ms in samples
            result.HasSustainedClipping = result.MaxConsecutiveClippedSamples > sustainedThreshold;
        }

        /// <summary>
        /// Assesses clipping quality and adds appropriate penalties and issues
        /// </summary>
        private static void AssessClippingQuality(ClippingAnalysisResult result, int sampleRate)
        {
            // Calculate clipping penalty - only flag significant clipping that indicates damage
            if (result.ClippingPercentage > SignificantClippingPercent) // Only penalize clipping above 5% (significant damage)
            {
            ...
            else if (result.ClippingPercentage > ModerateClippingPercent) // Flag only more significant clipping
            {
            ...
            if (result.TruePeakLevel > ExtremePeakThreshold) // Only flag peaks above +1 dBFS
            {
        ...
        private List<(int Start, int Length)> FindQuietPassages(AudioData audioData)
        {
            ...
            for (int i = 0; i + windowSize < audioData.LeftChannel.Length; i += windowSize / 2)
            {
                ...
                var windowRms = CalculateWindowRms(audioData, i, windowSize);
        ...
        /// <summary>
        /// Calculates RMS for a specific window of audio data
        /// </summary>
        private static double CalculateWindowRms(AudioData audioData, int start, int windowSize)
        {
            var windowRms = 0.0;
            ...
            return Math.Sqrt(windowRms / windowSize);
        }
        ...
        private void DetectNoiseTypes(NoiseFloorResult result, AudioData audioData)
        {
            ...
            if (result.NoiseFloorLevel > PoorNoiseFloorDb) // Only flag really poor noise floors above -30 dB
            {
                result.Issues.Add("High noise floor suggests poor recording conditions or equipment");
            }
            
            if (result.NoiseVariation > NoiseVariationThreshold)
            {
        ...
        private void CalculateNoiseFloorScore(NoiseFloorResult result)
        {
            ...
            if (result.NoiseFloorLevel > PoorNoiseFloorDb)
            {
                result.NoiseFloorScore *= 0.6f;
                result.Issues.Add($"Very poor noise floor ({result.NoiseFloorLevel:F1} dB)");
            }
            else if (result.NoiseFloorLevel > AcceptableNoiseFloorDb)
            {
        ...
        private bool CheckIfMono(AudioData audioData)
        {
            ...
            {
                if (Math.Abs(audioData.LeftChannel[i] - audioData.RightChannel[i]) > MonoThreshold)
                {
                    differences++;
                }
            }
            ...
            return differencePercentage < MonoThreshold; // Less than 1% differences = mono
        }
        ...
        private void AnalyzeStereoCharacteristics(ChannelQualityResult result, AudioData audioData)
        {
            ...
            CalculateChannelStatistics(audioData, sampleCount, ref correlationSum, ref leftSum, ref rightSum, ref leftSqSum, ref rightSqSum);

            // Calculate derived metrics
            CalculateChannelMetrics(result, correlationSum, leftSum, rightSum, leftSqSum, rightSqSum, sampleCount);

            // Analyze channel issues
            AnalyzeChannelIssues(result);
        }

        /// <summary>
        /// Calculates basic channel statistics
        /// </summary>
        private static void CalculateChannelStatistics(AudioData audioData, int sampleCount, 
            ref double correlationSum, ref double leftSum, ref double rightSum, ref double leftSqSum, ref double rightSqSum)
        {
        ...
        /// <summary>
        /// Calculates channel quality metrics from statistics
        /// </summary>
        private static void CalculateChannelMetrics(ChannelQualityResult result, double correlationSum, 
            double leftSum, double rightSum, double leftSqSum, double rightSqSum, int sampleCount)
        {
            // Channel correlation
        ...
        /// <summary>
        /// Analyzes various channel quality issues
        /// </summary>
        private static void AnalyzeChannelIssues(ChannelQualityResult result)
        {
            // Check for pseudo-stereo (high correlation but not identical)
            result.IsPseudoStereo = result.ChannelCorrelation > PseudoStereoCorrelation && result.ChannelCorrelation < MonoCorrelationThreshold;

            // Check for phase issues
            result.HasPhaseIssues = result.ChannelCorrelation < PhaseIssueThreshold;
            result.PhaseCoherence = Math.Abs(result.ChannelCorrelation);

            // Check for channel imbalance
            result.HasChannelImbalance = Math.Abs(result.LeftRightBalance) > ChannelImbalanceThreshold; // More than 20% imbalance

            // Check for polarity issues
            result.HasChannelPolarity = result.ChannelCorrelation < SeverePhaseIssueThreshold;
        }
        ...
        private void CalculateChannelQualityScore(ChannelQualityResult result)
        {
            ...
            ApplyChannelQualityPenalties(result);

            result.ChannelScore = Math.Max(0, result.ChannelScore);
        }

        /// <summary>
        /// Applies appropriate penalties for channel quality issues
        /// </summary>
        private static void ApplyChannelQualityPenalties(ChannelQualityResult result)
        {
            ...
            if (result.StereoWidth < NarrowStereoThreshold)
            {
                result.ChannelScore -= 10;
